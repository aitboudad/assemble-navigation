...
var navMethods = function(){
  ...
  function growBranch(menu, pathArr, menuItem, linkId){
    ...
    if(!_(menu).find({linkId: linkId})){
      ...
      if (!dirRef.hasOwnProperty('title')) { 
        dirRef.title = dirname;
        dirRef.items = [];
        dirRef.url = [];
      }
    }
    ...
    if(pathArr.length===0){
      ...
      if(menuItem.basename === 'index'){
        ...
        if ('_base' == menuItem['basename']) {
            dirRef['title'] = menuItem['title'];
        } else {
      ...
      growBranch(dirRef.items, pathArr, menuItem, linkId);
    }
  ...
  function crawPath(pathArr, page, menu, branch, linkId){
    ...
    if (dirRef['items']) {
        for (var i = 0; i < dirRef['items'].length; i++) {
          if (dirRef['items'][i]['basename'] == page.basename) {
              dirRef['items'][i]['isActive'] = true;
          }
        }
    }
